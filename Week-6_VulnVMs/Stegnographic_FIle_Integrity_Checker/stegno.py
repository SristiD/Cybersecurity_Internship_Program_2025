...
def encode_vid_data():
    cap=cv2.VideoCapture("Sample_cover_files/cover_video.xvid.avi")
    vidcap = cv2.VideoCapture("Sample_cover_files/cover_video.xvid.avi")    
    ...
    out_name = input("Enter name of output stego video (with extension, e.g. myvideo.avi): ")
    out = cv2.VideoWriter(out_name, fourcc, 25.0, size)
    ...
    print(f"\nEncoded the data successfully in the video file: {out_name}")
    return frame_

# In[22]:

def decode_vid_data(frame_):
    file_name = input("Enter the stego video file name to decode: ")
